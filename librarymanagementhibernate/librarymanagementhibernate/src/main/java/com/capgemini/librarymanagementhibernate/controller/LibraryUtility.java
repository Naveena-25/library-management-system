package com.capgemini.librarymanagementhibernate.controller;
	...
	public static void libraryManagementUtility() {
		...
		do {
			...
			case 1:
				...
				try {
					LibraryUsers user = service.login(emailId, password);
						...
						do {
							...
							case 2:
								try {
									...
									System.out.println(String.format("%-10s %-20s %-25s %-15s %-15s %-25s %-5s",
											"USER ID", "USER NAME", "EMAIL ID", "MOBILE NUMBER", "ROLE",
											"NO.OF BOOKS BORROWED", "FINE"));

									for (LibraryUsers libraryUsers : userList) {
										System.out.println(String.format("%-10s %-20s %-25s %-15s %-15s %-25s %-5s",
												libraryUsers.getId(), libraryUsers.getName(), libraryUsers.getEmailId(),
												libraryUsers.getMobileNumber(), libraryUsers.getRole(),
							...
							default:
								System.err.println("Invalid Option, Choice Should be in Between 0-9");
								break;
							}
			...
			case 2:
				...
				try {
					...
					if (userLogin != null && userLogin.getRole().equalsIgnoreCase("user")) {
						...
						do {
							...
							case 1:
								...
								try {
									boolean changepassword = service.changePassword(id, password, newPassword);
									if (changepassword) {
							...
							default:
								System.err.println("Invalid Option, Choice Should Be in Between 0-5");
								break;
						...
						System.err.println("Invalid User Credentials, Role Doesn't Match");
					}
				} catch (LMSException e) {
					System.err.println(e.getMessage());
				}
